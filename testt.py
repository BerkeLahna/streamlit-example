...
def main():
    ...
    input_data = {
        ...
        'job': st.selectbox("Enter job:", ['blue-collar', 'services', 'admin.', 'entrepreneur', 'self-employed',
                                          'technician', 'management', 'student', 'retired', 'housemaid', 'unemployed']),
        'marital': st.selectbox("Enter marital status:", ['single', 'married', 'divorced']),
        'education': st.selectbox("Enter education level:",
                                  ['basic.4y', 'basic.6y', 'basic.9y', 'high.school', 'university.degree',
        ...
        'campaign': st.slider("Enter number of contacts performed during this campaign for this client:",
                              min_value=0, max_value=50, value=5),
        'pdays': st.slider("Enter number of days that passed after the client was last contacted from a previous campaign:",
                           min_value=0, max_value=1000, value=0),
        'previous': st.slider("Enter number of contacts performed before this campaign for this client:",
        ...
        'cons.conf.idx': st.slider("Enter consumer confidence index:", min_value=-50.0, max_value=100.0, value=0.0,
                                   step=0.1),
    ...
    # Add input boxes for incrementing or decrementing the values
    st.subheader("Increment/Decrement Values")
    increment_decrement = {
        'age': st.number_input("Age", value=input_data['age']),
        'duration': st.number_input("Call Duration (seconds)", value=input_data['duration']),
        'campaign': st.number_input("Number of Contacts during this Campaign", value=input_data['campaign']),
        'pdays': st.number_input("Days since Last Contact", value=input_data['pdays']),
        'previous': st.number_input("Number of Contacts before this Campaign", value=input_data['previous']),
    }

    # Update the input_data dictionary with the incremented/decremented values
    input_data.update(increment_decrement)
